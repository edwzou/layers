...
const EditClothing = () => {
    ...
    const [itemType, setItemType] = useState([
        { label: capitalizeFirstLetter(ClothingTypes.outerwear), value: ClothingTypes.outerwear },
        { label: capitalizeFirstLetter(ClothingTypes.tops), value: ClothingTypes.tops },
        { label: capitalizeFirstLetter(ClothingTypes.bottoms), value: ClothingTypes.bottoms },
        { label: capitalizeFirstLetter(ClothingTypes.shoes), value: ClothingTypes.shoes },
    ]);
        ...
        <View style={{ flex: 1 }}>
            <View style={{ marginHorizontal: GlobalStyles.layout.xGap, gap: GlobalStyles.layout.gap }}>
                ...
                <View style={{ flexDirection: 'row', justifyContent: 'space-between' }}>
                    <View style={{ width: ITEM_SIZE(2) }}>
                        <Dropdown label='Item type' open={itemTypeOpen} setOpen={setItemTypeOpen} setItems={setItemType}
                            setValue={setItemTypeValue} items={itemType} value={itemTypeValue} />
                    </View>
                    <View style={{ width: ITEM_SIZE(2) }}>
                        <Dropdown label='Size' open={sizeOpen} setOpen={setSizeOpen} setItems={setSizes}
                            setValue={setSizeValue} items={sizes} value={sizeValue} />
                    </View>
                </View>
            </View>
            <SafeAreaView style={{ flex: 1 }}>
                ...
                <ScrollView contentContainerStyle={{ paddingBottom: GlobalStyles.layout.xGap * 6, paddingHorizontal: GlobalStyles.layout.xGap, marginTop: GlobalStyles.layout.gap, gap: GlobalStyles.layout.gap }}>
                    ...
                    <View style={{ flexDirection: 'row', flexWrap: 'wrap', flex: 1, gap: 5 }}>
                        <BrandTag label="Arc'teryx" action={TagAction.add} onPress={() => { console.log("Brand Click") }} />
                        <BrandTag label="yet" action={TagAction.remove} onPress={() => { console.log("Brand Click") }} />
                        <BrandTag label="Arc'teryx" action={TagAction.remove} onPress={() => { console.log("Brand Click") }} />
                    </View>
                </ScrollView>
            </SafeAreaView>
        </View>
